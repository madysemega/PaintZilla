...
export class LineService extends Tool {
  ...
  private isShapeCloseable(): boolean {
    let shapeHasEnoughVertices = this.vertices.length >= 3;
    if(!shapeHasEnoughVertices) {
      return false;
    }

    const CLOSING_SEGMENT_LENGTH_VALIDITY_THRESHOLD: number = 20; // (in pixels)

    const firstVertex: Vec2 = this.vertices[0];
    const mousePosition: Vec2 = this.lastMousePosition;
    
    const closingSegmentLength: number = Math.sqrt(
      ((mousePosition.x - firstVertex.x) ** 2) +
      ((mousePosition.y - firstVertex.y) ** 2)
    );
