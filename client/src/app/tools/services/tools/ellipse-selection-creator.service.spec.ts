...
describe('EllipseSelectionToolService', () => {
    ...
    beforeEach(() => {
        ...
        ellipseSelectionManipulatorMock = jasmine.createSpyObj('EllipseSelectionManipulatorService', ['onKeyDown', 'onKeyUp', 'onMouseDown', 'onMouseMove', 'onMouseUp', 'initialize', 'stopManipulation']);
...
*/
    ...
    it('if shift and mouse left button are down the selection outline should be redrawn if isSelectionBeingManipulated returns false', () => {
        keyboardEvent = {
            key: 'Shift',
        } as KeyboardEvent;
    ...
    it('if shift is not down and mouse left button is down the selection outline should not be redrawn if isSelectionBeingManipulated returns false', () => {
        keyboardEvent = {
            key: 'm',
        } as KeyboardEvent;
    ...
    it('if shift is released while mouse left button is down the selection outline should be redrawn if isSelectionBeingManipulated returns false', () => {
        keyboardEvent = {
            key: 'Shift',
        } as KeyboardEvent;
        service.mouseDown =true;
    ...
    it('if shift is not released while mouse left button is down the selection outline should not be redrawn if isSelectionBeingManipulated returns false', () => {
        keyboardEvent = {
            key: 'm',
        } as KeyboardEvent;
        service.mouseDown =true;
    ...
    it('if shift is down when creating a selection the endpoint should be adjusted with getSquareAjustedPerimeter', () => {
        let startPoint: Vec2 = {x:5, y:7};
        let endPoint: Vec2 = {x:10, y:9};
        service.isShiftDown =true;
    ...
    it('stop manipulating selection should delegate to EllipseSelectionManipulator and tell it to draw the selection if isSelectionBeingManipulatedSpy returns true;', () => {
        isSelectionBeingManipulatedSpy.and.returnValue(true);
        service.stopManipulatingSelection();
        expect(ellipseSelectionManipulatorMock.stopManipulation).toHaveBeenCalledWith(true);
    });

    it('stop manipulating selection should delegate to EllipseSelectionManipulator and tell it not to draw if isSelectionBeingManipulatedSpy returns false;', () => {
        isSelectionBeingManipulatedSpy.and.returnValue(false);
        service.stopManipulatingSelection();
    ...
    it('registerMousePosition should update mouseDownCoord and startPoint if isStartPoint is true', () => {
        let mousePos: Vec2 = {x:11, y:7};
        let oldValue: Vec2 = {x:5, y:5};
    ...
    it('adjustPositionToStayInCanvas should not change mousePos.x if mousePos.x >= 0 && mousePos.x < canvas width', () => {
        let mousePos: Vec2 = {x:5, y:7};
        let oldValue: number = mousePos.x;
        service.adjustPositionToStayInCanvas(mousePos);
    ...
    it('adjustPositionToStayInCanvas should make mousePos.x to equal canvas width if mousePose.x >= canvas width', () => {
        let width: number = drawServiceSpy.canvasSize.x;
        let mousePos: Vec2 = {x: width+1, y:7};
    ...
    it('adjustPositionToStayInCanvas should not change mousePos.y if mousePos.y >= 0  && mousePos.y < canvas height', () => {
        let mousePos: Vec2 = {x:5, y:7};
        let oldValue: number = mousePos.y;
    ...
    it('adjustPositionToStayInCanvas should make mousePos.y to equal canvas height if mousePose.y >= canvas height', () => {
        let height: number = drawServiceSpy.canvasSize.y;
        let mousePos: Vec2 = {x: 1, y: height+1};
    ...
    it('startPointIsFarEnoughFrom should return true if distance mouse pos <-> start point > minimum ', () => {
        let minimuDistance = service.MINIMUM_SELECTION_WIDTH;
        let mousePos: Vec2 = {x:5, y:7};
